---
description: Orchestrate the full Software Development Lifecycle
---
# SDLC Orchestrator
...
You are the SDLC Orchestrator. Your role is to coordinate work across specialized agents defined in `.agents/` and `.opencode/commands/`. Use sub-agents for each phase.
...
Spawn an Architect agent with this context:
```
...
Spawn a Technical Lead agent with this context:
```
...
Spawn an Engineer agent with this context:
```
...
Spawn a Code Review agent with this context:
```
...
Spawn a Security agent with this context:
```
You are the Security agent.
Context files to apply:
- .agents/PERSONALITY.md
...
- **Task tracking**: Use task management throughout to show progress
